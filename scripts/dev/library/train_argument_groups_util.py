...
def add_noise_loss_sampling_and_config_arguments(parser: argparse.ArgumentParser, support_dreambooth: bool) -> None:
    ...
    add("--timestep_sampling", type=str, default="uniform", choices=["uniform", "sigmoid", "shift"], help="timestep sampling method for U-Net training (default: uniform) / U-Net学習時のtime stepサンプリング方法（デフォルト: uniform）")
    add("--timestep_sigmoid_scale", type=float, default=1.0, help="scale factor used by sigmoid/shift timestep sampling (default: 1.0) / sigmoid/shiftのtime stepサンプリングで使うスケール係数（デフォルト: 1.0）")
    add("--timestep_shift", type=float, default=1.0, help="distribution shift used by shift timestep sampling, <1 biases lower timesteps and >1 biases higher timesteps (default: 1.0) / shiftサンプリングで使う分布シフト。1未満で低timestep寄り、1より大きいと高timestep寄り（デフォルト: 1.0）")
    add("--timestep_loss_weighting", type=str, default="none", choices=["none", "linear", "cosine", "sigmoid", "shift"], help="per-timestep loss weighting curve for U-Net training while keeping timestep sampling unchanged (default: none) / 在保持time step采样不变的前提下，为不同time step应用loss加权曲线（默认: none）")
    add("--timestep_loss_weight_sigmoid_scale", type=float, default=1.0, help="scale factor used by sigmoid/shift timestep loss weighting (default: 1.0) / sigmoid/shift的time step loss加权中使用的缩放系数（默认: 1.0）")
    add("--timestep_loss_weight_shift", type=float, default=1.0, help="distribution shift used by shift timestep loss weighting, <1 biases lower timesteps and >1 biases higher timesteps (default: 1.0) / shift型time step loss加权中使用的分布偏移。1未满更偏低timestep，1以上更偏高timestep（默认: 1.0）")
