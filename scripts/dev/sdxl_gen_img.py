...
from library.gen_img_sdxl_batch_util import process_sdxl_generation_batch
from library.gen_img_sdxl_input_util import load_sdxl_textual_inversion_inputs, prepare_sdxl_generation_inputs
from library.gen_img_sdxl_main_loop_util import run_sdxl_generation_iterations
from library.gen_img_sdxl_runtime_util import prepare_sdxl_runtime
...
def main(args):
    ...
    prepared_runtime = prepare_sdxl_runtime(
        args=args,
        dtype=dtype,
        logger=logger,
    ...
    if prepared_runtime is None:
        return
    text_encoder1 = prepared_runtime.text_encoder1
    text_encoder2 = prepared_runtime.text_encoder2
    vae = prepared_runtime.vae
    vae_dtype = prepared_runtime.vae_dtype
    unet = prepared_runtime.unet
    tokenizer1 = prepared_runtime.tokenizer1
    tokenizer2 = prepared_runtime.tokenizer2
    scheduler = prepared_runtime.scheduler
    scheduler_num_noises_per_step = prepared_runtime.scheduler_num_noises_per_step
    noise_manager = prepared_runtime.noise_manager
    device = prepared_runtime.device
    networks = prepared_runtime.networks
    network_default_muls = prepared_runtime.network_default_muls
    network_pre_calc = prepared_runtime.network_pre_calc
    upscaler = prepared_runtime.upscaler
    control_nets = prepared_runtime.control_nets
    pipe = prepared_runtime.pipe
    ...
    prepared_inputs = prepare_sdxl_generation_inputs(
        args=args,
    ...
    predefined_seeds = prepared_inputs.predefined_seeds
    ...
    def process_batch(batch: List[BatchData], highres_fix, highres_1st=False):
        return process_sdxl_generation_batch(
            batch,
            highres_fix=highres_fix,
    ...
    run_sdxl_generation_iterations(
        args=args,
        ...
        highres_fix=highres_fix,
        handle_dynamic_prompt_variants_fn=handle_dynamic_prompt_variants,
