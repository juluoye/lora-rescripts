Schema.intersect([
    ...
    Schema.object({
        ...
        train_batch_size: Schema.number().min(1).default(2).description("批量大小。单卡/单进程时就是实际 batch；多卡/分布式时按全局 batch 解释，启动时会自动换算成每卡。数值越高显存占用越高。"),
    ...
    Schema.intersect([
        Schema.object({
            network_module: Schema.union(["networks.lora_lumina", "lycoris.kohya"]).default("networks.lora_lumina").description("训练网络模块"),
